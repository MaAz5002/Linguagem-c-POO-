...
bool remover(listaSequencial &lista , int pos){
    
    if(listaVazia(lista)){
        return false;
    }

    // se a posição for menor que 0 ou maior que o numero total de elementos da lista
...
//& - evita duplicação da lista, acessando diretamente
// const - impede a modificação da lista
...
int main() {
    ...
    cout << "Antes da remoção: ";
    for (int i = 0; i < lista.n; i++) cout << lista.A[i] << " ";
    cout << endl;

    remover(lista, 1); // remove o elemento da posição 1

    cout << "Depois da remoção: ";
    for (int i = 0; i < lista.n; i++) cout << lista.A[i] << " ";
    cout << endl;
